<div class="card-header mb-2">
    <h5 class="mb-0 h6">{{translate('Add Your Exclude Product Base Coupon')}}</h5>
</div>
...
<div class="product-choose-list">
    <div class="product-choose">
        <div class="form-group row">
            <label class="col-lg-3 control-label" for="name">{{translate('Exclude Product')}}</label>
            <div class="col-lg-9">
			@php 
			$all_products = array(0);
            foreach(json_decode($coupon->details) as $key => $details) {
                array_push($all_products, $details->product_id);
            }
			@endphp
                <select name="product_ids[]" class="form-control product_id aiz-selectpicker" data-live-search="true" data-selected-text-format="count" required multiple>
                 @foreach($products as $key => $product)
                        <option value="{{$product->id}}"
                                @if ($coupon->type=="exclude_product_base" && !in_array($product->id,$all_products))
                                    selected
								 @elseif ($coupon->type != "exclude_product_base" && in_array($product->id,$all_products))
                                    selected
                                @endif
